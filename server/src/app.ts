...
export async function buildServer(
  ...
  const unsubscribe = runtime.subscribe((delta) =>
    viewerHub.broadcastDelta(delta),
  );
...
function parseClientMessage(
  ...
  | { ok: true; message: ParsedClientMessage & ClientMessage }
...
function handleClientMessage(
  ...
  if (message.worldId !== runtime.worldId) {
    viewerHub.sendError(
      socket,
      "INVALID_MESSAGE",
      "Message worldId does not match the connection",
    );
    return;
  }
