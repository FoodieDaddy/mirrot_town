  ...
  createEntity(entityData: any, type: string): THREE.Object3D {
    ...
    if (model) {
      ...
      if (currentHeight > profile.maxHeight) {
        ...
        console.debug(
          `[Scale Normalization] ${assetId}: height ${currentHeight.toFixed(2)} exceeds ${profile.maxHeight}, scaling down to ${scaleFactor.toFixed(2)}`
        );
      }
      ...
      console.warn(
        `[EntityFactory] Placeholder created for entity: ${entityData.id}, assetId: ${assetId}, type: ${type}. Reason: Asset not found in registry.`,
        {
