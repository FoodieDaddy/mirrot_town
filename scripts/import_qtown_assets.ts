...
const mappings: AssetMapping[] = [
    ...
    { id: 'road_straight', type: 'road', keywords: ['road_straight_crossing'], count: 1, destDir: 'roads', tags: ['road', 'straight'] },
    { id: 'road_corner', type: 'road', keywords: ['road_corner_curved'], count: 1, destDir: 'roads', tags: ['road', 'corner'] },
    { id: 'road_cross', type: 'road', keywords: ['road_intersection'], count: 1, destDir: 'roads', tags: ['road', 'cross'] },
    { id: 'road_t', type: 'road', keywords: ['road_tsplit'], count: 1, destDir: 'roads', tags: ['road', 't-junction'] },
    { id: 'road_plaza_tile', type: 'road', keywords: ['tileSmall_teamYellow'], count: 1, destDir: 'roads', tags: ['road', 'plaza'] },

    { id: 'nature_tree_round', type: 'nature', keywords: ['TwistedTree'], count: 3, destDir: 'nature', tags: ['nature', 'tree', 'round'] },
    { id: 'nature_tree_pine', type: 'nature', keywords: ['Pine_'], count: 2, destDir: 'nature', tags: ['nature', 'tree', 'pine'] },
    { id: 'nature_bush', type: 'nature', keywords: ['Bush_Common'], count: 3, destDir: 'nature', tags: ['nature', 'bush'] },
    { id: 'nature_rock_small', type: 'nature', keywords: ['RockPath_Square_Small'], count: 3, destDir: 'nature', tags: ['nature', 'rock'] },

    { id: 'npc_player', type: 'character', keywords: ['character_dog'], count: 1, destDir: 'characters', tags: ['character', 'player'] },
    { id: 'npc_base', type: 'character', keywords: ['character_bear', 'character_duck'], count: 3, destDir: 'characters', tags: ['character', 'npc'] },

    { id: 'prop_table', type: 'prop', keywords: ['Table_Spoon', 'Table_Round'], count: 1, destDir: 'props', tags: ['prop', 'furniture'] },
    { id: 'prop_chair', type: 'prop', keywords: ['Stool'], count: 1, destDir: 'props', tags: ['prop', 'furniture'] },
...
async function main() {
    ...
    for (const mapping of mappings) {
        ...
        potentialMatches.sort((a, b) => {
            ...
            const aExact = mapping.keywords.some(k => aName === `${k.toLowerCase()}.gltf` || aName === `${k.toLowerCase()}.glb`) ? 1 : 0;
            const bExact = mapping.keywords.some(k => bName === `${k.toLowerCase()}.gltf` || bName === `${k.toLowerCase()}.glb`) ? 1 : 0;
        ...
        for (let i = 0; i < potentialMatches.length && matched < mapping.count; i++) {
            ...
            if (success) {
                ...
                if (srcModel.includes('kaykit')) {
                    sourceName = 'KayKit'; license = 'CC0'; commercialUse = true;
                } else if (srcModel.includes('kenney')) {
                    sourceName = 'Kenney'; license = 'CC0'; commercialUse = true;
                } else if (srcModel.includes('quaternius')) {
                    sourceName = 'Quaternius'; license = 'CC0'; commercialUse = true;
                }
        ...
        if (matched < mapping.count) {
            console.warn(`⚠️  Missing required assets for ${mapping.id} (found ${matched}/${mapping.count})`);
        }
    }
